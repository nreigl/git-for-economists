...
Tasks economists perform today:
...
- **git add**: add new or modified files to the staging area 
 - **git add .**: adds all files excluding those defined in the .gitignore
 - **git add -u**: updates the index of an already staged file 
 - **git add -A**: adds all files **Including** those defined in the .gitignore
...
- `git pull` (`git fetch`; `git merge`): get last changes from a remote
- `git clone`: create local copy of public source project
>>>>>>> 2240b8b (Correct code markdown syntax)
...
Tips:
```bash
git config --global core.editor "code --wait" # sets VS Vode as core editor
git config --global -e
```
...
>>>>>>> ba4ad5b (add hyperlink to GUI Editors slide)
...
Cons:
 ...
 - In case of non-language environment -> no README.md, no lisence, no .gitignore
...
Cons:
 ...
 Always pull from the upstream repo *before* you push any changes!
=======
>>>>>>> 1dc6589 (Minor changes in ordering and language editing)
...
Fixing these conflicts is a simple matter of (manually) editing the conflicting file<sup>1</sup>.
